       ...
       <div class="panel-heading"><h3>Data Kategori</h3></div>
         <div class="panel-body">
           ...
           <tbody>
             ...
             <tr class="data-row">
               <td class="align-middle iteration">{{ $kategori->id_kategori }}</td>
               <td class="align-middle name">{{ $kategori->nama_kategori }}</td>
               <td>
                 <div class="row">
                   <div class="col-sm-2">
                     <button type="button" class="btn btn-warning" id="edit-item" data-item-id="{{ $kategori->id_kategori }}">edit</button>
                   </div>
                   <div class="col-sm-2" onclick="return confirm('Yakin Akan Menghapus?')">
                     {!! Form::open(['method' => 'DELETE', 'action' => ['KategoriController@destroy', $kategori->id_kategori], 'id'=>'delete']) !!}
    ...
    <div class="modal fade" id="edit-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            {!! Form::model($kategori, ['method' => 'PATCH', 'action' => ['KategoriController@update', $kategori->id_kategori], 'enctype' => 'multipart/form-data', 'id'=>'edit-form']) !!}

            @if ($errors->any())
              <div class="form-group {{ $errors->has('update_nama_kategori') ? 'has-error' : 'has-success'}}">
            @else
              <div class="form-group">
                @endif
                {!! Form::text('update_nama_kategori', null, ['class' => 'form-control', 'placeholder' => 'Nama Kategori', 'id' => 'modal-input-name']) !!}

                @if ($errors->has('update_nama_kategori'))
                  <span class="help-block" id="error-edit">{{ $errors->first('update_nama_kategori') }}</span>
                  <script>
                    $(document).ready(function(){
  ...
  <script type="text/javascript">
      $(document).ready(function() {
        ...
        $('#edit-modal').on('show.bs.modal', function() {
          ...
          $("#edit-form").attr("action", "/admin/kategori/"+ids);
        })
